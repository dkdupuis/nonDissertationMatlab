function reGenerateParFiles(fileLoc)

%Ntimes Ntrials InitialTime FinalTime
%MaxLag Niterations Tresh1 Tresh2
%'819',num2str(numTrials),'383','466','2','100','-1','.001'
...
for i=3:1:numFolders
    ...
    if(folders(i).isdir==1)
        ...
        oldPar=fopen(strcat(dataBaseLoc,folders(i).name,'\par.doc'));
        a= fscanf(oldPar, '%f');
        fclose(oldPar);
        folders(i)
        Ntimes=a(1);
        ...
        %sigLength
        wout=fopen(strcat(dataBaseLoc,folders(i).name,'\wout.doc'));
        trialNum = fgetl(wout);
        ...
        newPar=fopen(strcat(dataBaseLoc,folders(i).name,'\par.doc'),'w');
